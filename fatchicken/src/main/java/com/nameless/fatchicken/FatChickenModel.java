...
public class FatChickenModel extends ChickenModel<FatChickenEntity> {
    private final ModelPart head, beak, redThing, body, rightWing, leftWing;

    public FatChickenModel(ModelPart root) {
        super(root);
    ...
    @Override
    public void setupAnim(FatChickenEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        super.setupAnim(entity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch);

        // 嚴格還原你一開始代碼裡的數學邏輯
        float state2 = entity.getState() * 1.5F;
